@extends('layout')
@section('title','Inactivar Acta de COVID-19')
@section('titulopag','Administración')
@section('elcontrolador','Menu Princial')
@section('laaccion','INACTIVAR ACTA COVID-19.')
@section('content')
@csrf    
<div class="row">
...
<script>         
    $(function () {
        $("#menuInactivarEncuestaCovid").addClass("active" );

        var oActas = $("#tblActaCovidInhabilitar" ).DataTable({
        ...
        oActas.on('click','button.btn-inactivar', function () {
           if (typeof $(this).attr("data-id") != 'undefined') {
               ...
               $.fn.jAlert.defaults.title = 'Inactivar el Acta Covid';
               $.fn.jAlert.defaults.confirmAutofocus = '.denyBtn';
               confirm(function(e,btn){ //event + button clicked
                    e.preventDefault();
                    $.ajax({
                        type: "POST", url: "{{ route('acta.covid19.inactivar.ajax') }}",
                        data: { '_token': $('input[name=_token]').val(), 'id_formulario':id, },
                        success: function(response){
                            if(response.status=="1"){
                                oActas.draw(); successAlert('Inactivación',response.msg);
                            }else{
